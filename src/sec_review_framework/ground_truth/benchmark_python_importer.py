"""OWASP BenchmarkPython ground truth importer.

Ingests ``expectedresults-0.1.csv`` from a locally cloned BenchmarkPython
...
NOTE: BenchmarkPython test files are GPL-3.0.  This module intentionally
...
Pinned default ``origin_commit``:
    9f0d34945a8872220957a4f99cb2721cd9036a6b
    (HEAD of OWASP-Benchmark/BenchmarkPython main as of 2026-04-28)
"""
...
@dataclass
class ImportResult:
    """Summary returned by :func:`import_benchmark_python`."""
...
def _load_category_map() -> dict[str, str]:
    ...
    Every key in the YAML whose name matches a BenchmarkPython category is
    eligible — no special-casing required because A7 added those keys directly
    to the YAML.
    """
...
def _category_to_vuln_class(category: str, category_map: dict[str, str]) -> str:
    """Map a BenchmarkPython CSV category to a ``vuln_class`` string.
    ...
    try:
        ...
        raise ValueError(
            f"Unknown BenchmarkPython category {category!r}. "
...
_DEFAULT_ORIGIN_COMMIT = "9f0d34945a8872220957a4f99cb2721cd9036a6b"


async def import_benchmark_python(
    db: Database,
    *,
    origin_url: str = "https://github.com/OWASP-Benchmark/BenchmarkPython",
    origin_commit: str = _DEFAULT_ORIGIN_COMMIT,
    csv_path: Path,
    testcode_dir: Path,
    version: str = "0.1",
    dataset_name: str = "owasp-benchmark-python-v0.1",
) -> ImportResult:
    """Import OWASP BenchmarkPython ground truth into the framework database.
    ...
    Args:
        ...
        origin_url: Canonical GitHub URL of the BenchmarkPython repo.
        origin_commit: Pinned SHA of the cloned working tree.
        csv_path: Path to ``expectedresults-0.1.csv`` in the cloned tree.
        testcode_dir: Path to the ``testcode/`` directory (used for line counts).
        version: Human-readable benchmark version string (default ``"0.1"``).
        ...
        await db.create_dataset(
            {
                ...
                "metadata_json": json.dumps(
                    {
                        "benchmark": "owasp-python",
                        "version": version,
                        "iteration": "per-test-file",
                        "test_glob": "testcode/BenchmarkTest*.py",
                    }
                ),
    ...
    for row in parsed_rows:
        ...
        file_path = f"testcode/{test_name}.py"
        py_file = testcode_dir / f"{test_name}.py"
        ...
        if row["is_positive"]:
            ...
            # ------------------------------------------------------------------
            if py_file.exists():
                line_count = len(py_file.read_text(errors="replace").splitlines())
