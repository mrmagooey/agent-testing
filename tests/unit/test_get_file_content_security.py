...
def _make_coordinator(tmp_path: Path) -> tuple[Coordinator, Path]:
    ...
    coord.get_labels = AsyncMock(return_value=[])
...
@pytest.mark.asyncio
async def test_rejects_path_traversal_single_dotdot(tmp_path: Path):
    coord, _ = _make_coordinator(tmp_path)
    outside = tmp_path / "secret.txt"
    outside.write_text("secret")
    with pytest.raises(HTTPException) as exc_info:
        await coord.get_file_content("myds", "../secret.txt")
    assert exc_info.value.status_code == 400
    assert "escapes dataset" in exc_info.value.detail


@pytest.mark.asyncio
async def test_rejects_path_traversal_nested_dotdot(tmp_path: Path):
    coord, _ = _make_coordinator(tmp_path)
    with pytest.raises(HTTPException) as exc_info:
        await coord.get_file_content("myds", "../../etc/passwd")
    assert exc_info.value.status_code == 400
...
@pytest.mark.asyncio
async def test_rejects_nul_byte(tmp_path: Path):
    coord, _ = _make_coordinator(tmp_path)
    with pytest.raises(HTTPException) as exc_info:
        await coord.get_file_content("myds", "safe.py\x00/etc/passwd")
    assert exc_info.value.status_code == 400
    assert "NUL" in exc_info.value.detail


@pytest.mark.asyncio
async def test_rejects_symlink_escape(tmp_path: Path):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await coord.get_file_content("myds", "escape_link/secret.txt")
    assert exc_info.value.status_code == 400
...
@pytest.mark.asyncio
async def test_binary_detection(tmp_path: Path):
    coord, dataset_dir = _make_coordinator(tmp_path)
    bin_file = dataset_dir.parent / "data.bin"
    bin_file.write_bytes(b"ELF\x7f\x00binary\x00data")
    result = await coord.get_file_content("myds", "data.bin")
...
@pytest.mark.asyncio
async def test_binary_detection_nul_after_8kb(tmp_path: Path):
    ...
    result = await coord.get_file_content("myds", "almost_text.txt")
...
@pytest.mark.asyncio
async def test_large_file_truncation(tmp_path: Path):
    coord, dataset_dir = _make_coordinator(tmp_path)
    big_file = dataset_dir.parent / "huge.py"
    big_file.write_bytes(b"x" * (3 * 1024 * 1024))  # 3 MiB > 2 MiB limit
    result = await coord.get_file_content("myds", "huge.py")
...
@pytest.mark.asyncio
async def test_small_file_not_truncated(tmp_path: Path):
    coord, dataset_dir = _make_coordinator(tmp_path)
    small_file = dataset_dir.parent / "small.py"
    small_file.write_text("print('hello')\n")
    result = await coord.get_file_content("myds", "small.py")
...
@pytest.mark.asyncio
async def test_labels_attached_for_matching_path(tmp_path: Path):
    coord, dataset_dir = _make_coordinator(tmp_path)
    ...
    coord.get_labels = AsyncMock(return_value=[label])
    result = await coord.get_file_content("myds", "src/auth.py")
    assert len(result["labels"]) == 1
    assert result["labels"][0]["label_id"] == "lbl-1"


@pytest.mark.asyncio
async def test_labels_not_attached_for_different_path(tmp_path: Path):
    coord, dataset_dir = _make_coordinator(tmp_path)
    src_file = dataset_dir.parent / "other.py"
    src_file.write_text("pass\n")
    label = {"label_id": "lbl-2", "file_path": "src/auth.py"}
    coord.get_labels = AsyncMock(return_value=[label])
    result = await coord.get_file_content("myds", "other.py")
...
@pytest.mark.asyncio
async def test_highlight_params_echoed(tmp_path: Path):
    coord, dataset_dir = _make_coordinator(tmp_path)
    f = dataset_dir.parent / "a.py"
    f.write_text("line1\nline2\nline3\n")
    result = await coord.get_file_content("myds", "a.py", start=2, end=3)
    assert result["highlight_start"] == 2
    assert result["highlight_end"] == 3


@pytest.mark.asyncio
async def test_highlight_params_absent_when_not_given(tmp_path: Path):
    coord, dataset_dir = _make_coordinator(tmp_path)
    f = dataset_dir.parent / "b.py"
    f.write_text("line1\n")
    result = await coord.get_file_content("myds", "b.py")
...
@pytest.mark.asyncio
async def test_missing_file_raises_404(tmp_path: Path):
    coord, _ = _make_coordinator(tmp_path)
    with pytest.raises(HTTPException) as exc_info:
        await coord.get_file_content("myds", "nonexistent.py")
...
@pytest.mark.asyncio
async def test_rejects_invalid_dataset_name(tmp_path: Path, bad_name: str):
    coord, _ = _make_coordinator(tmp_path)
    with pytest.raises(HTTPException) as exc_info:
        await coord.get_file_content(bad_name, "anything.py")
    assert exc_info.value.status_code == 400
