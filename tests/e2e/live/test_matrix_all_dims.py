...
Two 1-cell batches × 2 repetitions each = 4 runs total. Every experiment
...
CELL_B_PAYLOAD = {
    ...
    "max_batch_cost_usd": 0.60,
    "strategy_configs": {"per_file": {"max_turns": 3}},
}


def test_matrix_all_dims(live_client, batch_cleanup):
    # --- Submit Cell A ---
    batch_id_a = unique_batch_id("live-e2e-all-dims-a")
    batch_cleanup.append(batch_id_a)
    resp_a = live_client.post("/batches", json={**CELL_A_PAYLOAD, "batch_id": batch_id_a})
    assert resp_a.status_code == 201, f"Cell A submit failed {resp_a.status_code}: {resp_a.text}"
    body_a = resp_a.json()
    assert body_a["batch_id"] == batch_id_a
    assert body_a["total_runs"] == 2, (
        f"Cell A: expected 2 runs (1 cell × 2 reps), got {body_a['total_runs']}"
    )

    # --- Submit Cell B ---
    batch_id_b = unique_batch_id("live-e2e-all-dims-b")
    batch_cleanup.append(batch_id_b)
    resp_b = live_client.post("/batches", json={**CELL_B_PAYLOAD, "batch_id": batch_id_b})
    assert resp_b.status_code == 201, f"Cell B submit failed {resp_b.status_code}: {resp_b.text}"
    body_b = resp_b.json()
    assert body_b["batch_id"] == batch_id_b
    assert body_b["total_runs"] == 2, (
        f"Cell B: expected 2 runs (1 cell × 2 reps), got {body_b['total_runs']}"
    )

    # --- Poll Cell A to completion ---
    # Both batches run concurrently on the cluster; polling is sequential.
    # Cell A's timeout is deliberately tight (600 s) — it's the cheap cell,
    # and a hang here must not starve Cell B's poll budget.
    final_a = poll_until_done(live_client, batch_id_a, timeout_s=600, poll_interval_s=15)
    ...
    final_b = poll_until_done(live_client, batch_id_b, timeout_s=1200, poll_interval_s=15)
    ...
    # --- Fetch results for both batches — plumbing check only ---
    results_a = live_client.get(f"/batches/{batch_id_a}/results")
    ...
    results_b = live_client.get(f"/batches/{batch_id_b}/results")
