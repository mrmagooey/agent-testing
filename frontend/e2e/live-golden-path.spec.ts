 ...
 *
 * Submit approach: Phase B uses `request.post('/api/batches')` directly rather
...
test.describe('@live golden path', () => {
  ...
  // Phase C may need up to 10 minutes for the batch to reach a terminal state.
  test.setTimeout(10 * 60 * 1000)

  test('submit batch, poll until terminal, inspect run detail', async ({ page, request }) => {
    let batchId: string
    ...
    await test.step('Phase A: dashboard loads', async () => {
      ...
      // Nav landmark is always present regardless of batch count
      await expect(page.getByRole('navigation')).toBeVisible()
    })

    // -----------------------------------------------------------------------
    // Phase B: Submit a batch via the API
    // -----------------------------------------------------------------------
    await test.step('Phase B: submit batch via API', async () => {
      const bid = uniqueBatchId('live-ui')
      const payload = {
        batch_id: bid,
      ...
      const resp = await request.post('/api/batches', { data: payload })
      // The coordinator returns 201 on success; accept both 200 and 201.
      expect(resp.status()).toBeGreaterThanOrEqual(200)
      expect(resp.status()).toBeLessThan(300)

      const body = await resp.json()
      // Grab whatever batch_id the server assigned (may echo ours or generate one)
      batchId = (body.batch_id as string) ?? bid

      // Navigate to the dashboard and confirm the batch appears. The dashboard
      // shows truncated batch IDs (first 8 chars + ellipsis). Prior test runs
      // may leave other batches with the same short prefix, so `.first()`
      // avoids strict-mode violations while still proving the new batch landed.
      await page.goto('/')
    ...
    await test.step('Phase C: poll until terminal and inspect run detail', async () => {
      ...
      while (Date.now() < deadline) {
        const resp = await request.get(`/api/batches/${batchId}`)
        expect(resp.ok()).toBe(true)
        const batch = await resp.json()
        finalStatus = batch.status as string
      ...
      // Navigate to the batch detail page in the browser. BatchDetail renders
      // the batch_id as the primary <h1> once the batch fetch completes.
      await page.goto(`/batches/${batchId}`)
      await expect(
        page.getByRole('heading', { name: batchId })
      ).toBeVisible({ timeout: 30_000 })

      // Fetch the run list from the API to get a run ID for the detail page.
      const runsResp = await request.get(`/api/batches/${batchId}/runs`)
      expect(runsResp.ok()).toBe(true)
      const runs = await runsResp.json() as Array<{ id: string; batch_id?: string; status?: string; prompt_tokens?: number; completion_tokens?: number; cost_usd?: number }>
      ...
      await page.goto(`/batches/${batchId}/runs/${runId}`)
      ...
      // proves the SPA fetched /api/batches/{id}/runs/{runId} and rendered it.
